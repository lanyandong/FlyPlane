...
bool OverScene::init(int score) {
	do
	{
		...
		auto bg = Sprite::create("gameover1.png");
...
//		log(UserDefault::getInstance()->getXMLFilePath().c_str());
		...
		lblHightScore->setPosition(VISIBLE_ORIGIN + Vec2(VISIBLE_SIZE.width / 2, VISIBLE_SIZE.height * 3 / 5));
		this->addChild(lblHightScore, UI_ZORDER);
		lblHightScore->setColor(Color3B::BLACK);
		lblHightScore->setBMFontSize(60);
		if (highScore<score)
		{
			AudioEngine::play2d("achievement.mp3", false, 0.5f);
			highScore = score;
		...
		lblScore->setPosition(VISIBLE_ORIGIN + Vec2(VISIBLE_SIZE.width / 2, VISIBLE_SIZE.height / 5));
